...
$$
\Delta _{\Pi (i)} := \min \limits_{S_j \in \Pi \setminus \{ S_{\Pi (i)} \}} 

\max \limits_{\Delta \in \mathbb {Z}}\lbrace \Delta \mid (t_{\Pi (i)},|{S}_{\Pi (i)}| + \Delta)\succeq _i (t_j,|{S}_j \cup \lbrace a_i\rbrace |)\rbrace . \tag{4}
$$
...
\min \limits_{S_j \in \Pi \setminus \{ S_{\Pi (i)}\} } 
