...
考虑一个对称寡头垄断的 Cournot 竞争，其成本函数为线性函数 $c_{i}\left(q_{i}\right)=cq_{i}$ ，$1\leq i\leq n$。逆需求函数为 $F(Q)$ ，$Q>0$，是一个正函数（不需要对 $F$ 进行单调性、连续性或可微性假设）。企业 $i$ 的利润函数定义为：

$$\Pi_i(q_1,q_2,\ldots,q_n)=F(Q)q_i-cq_i,$$

其中 $Q=\sum_{j=1}^{n}q_{j}$。定义一个函数 $P$ ：$R_{++}^{n}\longrightarrow R$：
...
满足 (1.1) 的函数 $P$ 被称为序数势函数，而具有序数势函数的博弈被称为序数势博弈。显然，Cournot 博弈的纯策略均衡集与每个企业的利润由 $P$ 给出的博弈的纯策略均衡集一致。如果我们对混合策略感兴趣，则需要比 (1.1) 更强的条件。
